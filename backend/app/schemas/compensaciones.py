...
class CompensacionCreate(BaseModel):
    ...
    `moneda_deuda` dice contra qué deudas del cliente se imputa: ARS y USD son
    cajas distintas y no se suman (los cheques fiados son siempre ARS).
    """
...
class CompensacionResponse(BaseModel):
    ...
    nueva. `excedente` es lo que el cliente transfirió de más sobre lo que
    debía: le queda a favor como deuda del negocio con él (mismo criterio que el
    vuelto de un cheque, §5), en la moneda en que transfirió.
    """
