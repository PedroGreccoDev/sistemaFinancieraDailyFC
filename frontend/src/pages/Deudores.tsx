...
const semaforoStyle: Record<Semaforo, { border: string; badge: string; label: string }> = {
  mora:     { border: 'border-red-400 dark:border-red-500',    badge: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400',       label: 'En mora' },
  proximo:  { border: 'border-yellow-400 dark:border-yellow-500', badge: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-400', label: 'Vence pronto' },
  ok:       { border: 'border-green-400 dark:border-green-600',  badge: 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400',   label: 'Al día' },
  cancelado:{ border: 'border-slate-200 dark:border-slate-700', badge: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400',    label: 'Cancelado' },
}
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">Deudores</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        {activos.map((p) => {
          ...
          return (
            <div key={p.id} className={`bg-white dark:bg-slate-800 border-2 ${style.border} rounded-lg p-4 shadow-sm`}>
              <div className="flex items-start justify-between mb-2">
                <h3 className="font-semibold text-slate-900 dark:text-slate-100 truncate">{nombre}</h3>
              ...
              <div className="text-sm text-slate-600 dark:text-slate-400 space-y-1 mb-3">
                <div className="flex justify-between">
                  <span>Capital</span>
                  <span className="font-medium text-slate-800 dark:text-slate-200">{fmtMonto(p.credito, p.moneda)}</span>
                </div>
                <div className="flex justify-between">
      ...
      {cancelados.length > 0 && (
        <details className="group">
          <summary className="cursor-pointer text-sm text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 select-none list-none flex items-center gap-2">
          ...
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {cancelados.map((p) => {
              const nombre = clienteMap.get(p.cliente_id) ?? '…'
              return (
                <div key={p.id} className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-4 opacity-60">
                  <div className="flex justify-between items-center">
                    <span className="font-medium text-slate-700 dark:text-slate-300">{nombre}</span>
                    <span className="text-xs bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 rounded-full px-2 py-0.5 capitalize">{p.estado}</span>
                  </div>
